...
export async function fetchMergeStatus(prApiUrl) {
  try {
...
export async function fetchOwnerDetails(ownerName) {
  try {
...
export async function fetchRepositoryDetails(ownerName, repoName) {
  try {
    const response = await axios.get(
...
export async function fetchUserDetailsById(github_id) {
  try {
    const response = await axios.get(
    ...
    console.error(
      `Error fetching details for user ${github_id}:`,
      error.message
    )
    return null
  }
...
export async function fetchPRsForUser(username) {
  let prRows = []
  let page = 1
